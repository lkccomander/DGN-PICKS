...
export default function Home() {
  ...
  return (
    <main className="app-shell">
      ...
      <div className="landing-toolbar"><label className="toolbar-search"><span aria-hidden="true">⌕</span><span className="sr-only">Search the board</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search teams, games or picks" /></label><div className="toolbar-spacer" /><label className="quick-bet"><span>QUICK BET</span><input aria-label="Quick bet tokens" inputMode="decimal" placeholder="0" /></label><button type="button" className="toolbar-toggle" aria-label="Toggle theme">☼ <span /></button><label className="toolbar-select"><span className="sr-only">Odds format</span><select defaultValue="american"><option value="american">American Odds</option><option value="decimal">Decimal</option><option value="fractional">Fractional</option></select></label><label className="toolbar-select"><span className="sr-only">Language</span><select defaultValue="en"><option value="en">EN</option><option value="es">ES</option></select></label><span className="toolbar-clock">16:21:40 (GMT-06:00)</span><a className="toolbar-help" href="#help">? &nbsp; Help</a></div>
